-- ==============================================================================
-- PRODUCTION MULTI-TENANT ISOLATION MIGRATION SCRIPT (FINAL AUDITED RELEASE)
...
-- ==============================================================================
-- 5. SAFELY MIGRATE subscription_licenses (STRICT RELATIONSHIP & ORPHAN SAFEGUARD)
-- ==============================================================================
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'subscription_licenses') THEN
    ...
    -- 5.2 Derive business_id from subscriptions where subscription_id exists and matches
    ...
    -- 5.3 Enforce foreign key only when business_id IS NOT NULL (preserves any genuine unassigned orphan records without guessing ownership)
...
-- ==============================================================================
-- 7. SECURE BUSINESS REGISTRATION RPC (COLLISION-PROOF & HIJACK-RESISTANT)
-- ==============================================================================
...
BEGIN
  ...
  -- 7.1 Hijack Prevention: Reject registration if user already has an active business assignment
  ...
  -- 7.2 Collision-Safe Cryptographic Identifier Generation
  v_clean_name := LOWER(REGEXP_REPLACE(p_business_name, '[^a-zA-Z0-9]+', '-', 'g'));
  v_clean_name := TRIM(BOTH '-' FROM v_clean_name);
  IF LENGTH(v_clean_name) = 0 THEN v_clean_name := 'biz'; END IF;
  IF LENGTH(v_clean_name) > 30 THEN v_clean_name := SUBSTRING(v_clean_name FROM 1 FOR 30); END IF;

  v_clean_code := UPPER(REGEXP_REPLACE(COALESCE(p_business_code, 'BIZ'), '[^A-Z0-9]', '', 'g'));
  v_random_entropy := SUBSTRING(ENCODE(GEN_RANDOM_BYTES(6), 'hex') FROM 1 FOR 8);
  
  v_biz_id := 'biz-' || v_clean_name || '-' || TO_CHAR(NOW(), 'YYYYMMDD') || '-' || v_random_entropy;
  v_sub_id := 'sub-' || TO_CHAR(NOW(), 'YYYYMMDD') || '-' || SUBSTRING(ENCODE(GEN_RANDOM_BYTES(4), 'hex') FROM 1 FOR 6);

  -- 7.3 Create Business Record (PENDING_PAYMENT)
  INSERT INTO public.businesses (
  ...
  -- 7.4 Link User Profile (Locked to Admin role, is_super_admin = FALSE)
  ...
  -- 7.5 Create Initial Subscription Record
  ...
  -- 7.6 Initialize hotel_store with baseline tenant configuration
  ...
  VALUES 
    ...
    (v_biz_id, 'app_version', jsonb_build_object('version', '3.1.0', 'initialized_at', NOW()), NOW())
...
BEGIN
  IF NOT public.is_super_admin() THEN
    ...
    -- Prevent modifying assigned business ID once set
...
-- 9.1 BUSINESSES (Direct INSERT closed; creation allowed exclusively via register_business_secure or Super Admin)
...
-- 9.2 PROFILES (Direct INSERT strictly limited to Super Admin or registration workflow; business_id assignment protected)
...
WITH CHECK (
  public.is_super_admin() OR (
    id = auth.uid() AND 
    (business_id IS NULL OR business_id = public.get_auth_business_id()) AND 
...
DROP POLICY IF EXISTS "Profiles delete policy" ON public.profiles FOR DELETE TO authenticated
USING (public.is_super_admin());
...
-- 9.6 AUDIT LOGS (Anti-Spoofing: Enforces authentic business_id and auth.uid())
...
WITH CHECK (
  public.is_super_admin() OR (
    business_id = public.get_auth_business_id() AND 
